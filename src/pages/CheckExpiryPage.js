...
const CheckExpiryPage = () => {
  ...
  const getCompanyName = (companyID) => {
    console.log("Companies:", companies); // Debug companies data
    const company = companies.find((c) => c.companyID === companyID);
    return company ? company.companyName : "Unknown";
  };

  const getProductName = (productID) => {
    console.log("Products:", products); // Debug products data
  ...
  const currentDate = new Date(); // Dynamic current date and time (June 30, 2025, 10:25 AM EAT)
  ...
  const nearToExpireLicenses = licenses.filter((license) => {
    ...
    console.log(
      `License ${license.companyProductID}: ${daysDifference} days remaining`
    );
    return !license.isExpired && daysDifference <= 30 && daysDifference >= 0;
  });
  ...
  const validLicenses = licenses.filter((license) => {
    ...
    return !license.isExpired && daysDifference > 30;
  });

  const handleRenew = (companyProductID) => {
    // Prioritize renewal fee, fallback to subscription fee (e.g., $89), then "Unknown"
    const renewalFee = feeStructures.find((fee) => fee.feeType === "renewal");
    const subscriptionFee = feeStructures.find(
      (fee) => fee.feeType === "Subscription"
    );
    const feeAmount = renewalFee
      ? renewalFee.feeAmount
      : subscriptionFee
      ? subscriptionFee.feeAmount
      : "Unknown";
    const displayAmount = feeAmount !== "Unknown" ? `$${feeAmount}` : "Unknown";
    toast.info(`You will pay ${displayAmount} for license renewal`);
  };
        ...
        <Space>
          ...
          {expiredLicenses.some(
            ...
            <Button
              ...
              onClick={() => handleRenew(record.companyProductID)}
            >
              Renew
            </Button>
          )}
...
//   const handleRenew = (companyProductID) => {
//     const feeStructure = feeStructures.find((fee) => fee.feeType === "renewal");
//     const feeAmount = feeStructure ? feeStructure.feeAmount : "Unknown";
//     toast.info(`You will pay ${feeAmount} for license renewal`);
//   };
